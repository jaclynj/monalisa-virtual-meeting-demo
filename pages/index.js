...
const prepAndSortMeetings = (meetings) =>
  sortMeetings(filterPastMeetings(meetings));
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Fall back to meetingData.json when nothing is saved yet
    const initial = stored ? JSON.parse(stored) : meetingData.meetings;
    setMeetings(prepAndSortMeetings(initial));
  }, []);

  const addMeeting = (meeting) => {
    const next = prepAndSortMeetings([meeting, ...meetings]);
  ...
  return (
    <div className="container">
      <Head>
        <title>Virtual Meeting Tracker</title>
