...
const filterFuture = (meetings) =>
  meetings.filter((m) => new Date(`${m.meetingDate}T${m.meetingTime}`) - Date.now() > 0);

const prepAndSort = (meetings) => sortMeetings(filterFuture(meetings));
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const loaded = stored ? JSON.parse(stored) : meetingData.meetings;
    setMeetings(prepAndSort(loaded));
  }, []);

  const addMeeting = (meeting) => {
    setMeetings((prev) => {
      const next = prepAndSort([meeting, ...prev]);
      localStorage.setItem("localMeetingData", JSON.stringify(next));
      return next;
    });
  };
